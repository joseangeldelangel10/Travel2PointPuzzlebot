...
class trafficSignalsDetector():
    def __init__(self, cam_topic = "/video_source/raw", simulation = False):
        ...
        f = open("/home/puzzlebot/LeoNJosews/src/chrisAct1_1/CameraCalibration/perspactiveCorrectionData.json")
        data = json.load(f)    

        left_transformation_matrix = data["leftTransformationMat"]
        right_transformation_matrix = data["rightTransformationMat"]

        self.left_transformation_matrix = np.array(left_transformation_matrix)
        self.right_transformation_matrix = np.array(right_transformation_matrix)
        self.left_transformation_matrix = np.reshape(left_transformation_matrix, (3,3))
        self.right_transformation_matrix = np.reshape(right_transformation_matrix, (3,3))
        self.transformation_size = data["transformationSize"]        
    ...
    def main(self):
        while not rospy.is_shutdown():
            ...
            if self.image != None:
                ...
                (originalHeight, originalWidth, _) = self.cv_original_image.shape
                ...
                self.cv_image = cv.resize(self.cv_original_image,(self.newWidth,self.newHeight), interpolation = cv.INTER_AREA)
                
                one_third_width = int(self.newWidth*(5.0/12.0))
                two_thirds_width = int(self.newWidth*(7.0/12.0))
                left_side = self.cv_image[:,:one_third_width]
                right_side = self.cv_image[:,two_thirds_width:]
                result1 = cv.warpPerspective(left_side, self.left_transformation_matrix, (self.transformation_size[1], self.transformation_size[0]) , flags= cv.INTER_LINEAR)
                result2 = cv.warpPerspective(right_side, self.right_transformation_matrix, (self.transformation_size[1], self.transformation_size[0]) , flags= cv.INTER_LINEAR)                
                self.cv_image = np.concatenate((result1, result2), axis=1)

                self.final_detection = self.cv_image.copy()
                minImageArea = self.cv_image.shape[0]*self.cv_image.shape[1]*(1/250)
                ...
                for (x,y,w,h) in blueCircles:
                    ...
                    if (className == "straight sign" or className == "turn right ahead sign"):
                        cv.rectangle(self.final_detection, (x,y),(x+w,y+h),(255,0,0),2)
                        cv.putText(self.final_detection,'{class_name}, prob={prob:.2f}'.format(class_name=className, prob=probabilityValue),(x,y+h),2,0.7,(255,0,0),2,cv.LINE_AA)
                        self.curr_traffic_signs.append(className)
